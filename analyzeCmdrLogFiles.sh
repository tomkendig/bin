...
mkdir graphs
chmod 777 graphs
#zcat commander*.log* | awk --file=/net/f2home/tkendig/bin/scheduler.awk | tail -12 > analysis/scheduler.txt & #look for schedule statistics in the log file
#zegrep "Created session|stepSchedular" commander.2012-10-31.078.log.zip | egrep "ResourceSchedulerImpl|Created session" |  egrep "runProcedure#|runCommand" | less # look for data that allows you to see how long from start of job to when first step is created.
zcat commander*.log* | /net/f2home/tkendig/bin/gaps.py --verbose | grep gap | sort -k 4 -nur > analysis/gaps.txt & #look for gaps in the log file - often occuring during JRE garbage collection
zcat commander*.log* | /net/f2home/tkendig/bin/gaps.py --verbose --min 5 > analysis/gaps5min.txt & #look for gaps in the log file - often occuring during JRE garbage collection
zegrep -h 'dispatchApiRequest.perform elapsed' commander*.log* | grep DEBUG | awk -F'|' '{print $1 " " $8}' | awk '{print $1 " " $5}' | sed s/,//g > graphs/dispatchApiRequest.txt &#;gnuplot < ~tkendig/results/dispatchApiRequest.cmd > dispatchApiRequest.gif
zegrep -h 'getProperty.perform elapsed' commander*.log* | grep DEBUG | awk -F'|' '{print $1 " " $8}' | awk '{print $1 " " $5}' | sed s/,//g > graphs/getProperty.txt &#;gnuplot < ~tkendig/results/getProperty.cmd > getProperty.gif
zegrep -h 'findJobSteps.perform elapsed' commander*.log* | grep DEBUG | awk -F'|' '{print $1 " " $8}' | awk '{print $1 " " $5}' | sed s/,//g > graphs/findJobSteps.txt &#;gnuplot < ~tkendig/results/findJobSteps.cmd > findJobSteps.gif
zegrep -h 'cpuMonitor' commander*.log* | grep DEBUG | awk '{print $1 " " $14 " " $16 " " $18 " " $20 " " $22 " " $24}' > graphs/cpuMonitor.txt ; #ssh qa@ken-qa "gnuplot < ~tkendig/results/cpuMonitor.cmd > cpuMonitor.gif; gnuplot < ~tkendig/results/findJobSteps.cmd > findJobSteps.gif; gnuplot < ~tkendig/results/dispatchApiRequest.cmd > dispatchApiRequest.gif; gnuplot < ~tkendig/results/getProperty.cmd > getProperty.gif"
grep Swap 201* | awk -F':' '{print $1 " " $3}' | awk '{print $1 " " $4}' > graphs/Swap.txt ; #ssh qa@ken-qa "gnuplot < ~tkendig/results/Swap.cmd > Swap.gif"
/net/f2home/tkendig/bin/log-keywords.pl commander*.log* > analysis/log-keywords.txt & # look for default errors this script catches
...
#zegrep "post data|ping message" agent*.*log* > analysis/ping.txt #generate file where pings from commander server should line up
...
echo '################################ extra archived queries'
#zgrep "16.167.15.211" commander.2012-12-13.004.log.zip | grep XmlRequestHandler | wc
